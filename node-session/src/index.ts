...
import { ConfigInterface, JoinSessionBody } from './types'
...
const envSchema = z.object({
  ...
  SESSION_JOIN_PWD: z
    .string({ message: 'SESSION_JOIN_PWD é obrigatório' })
    .min(1, 'SESSION_JOIN_PWD não pode estar vazio'),
  PORT: z
    .string()
    .optional()
    .transform((val) => (val ? Number(val) : 3334)),
...
app.post('/create', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  const session = await WiseAPI.session.create({
    org: env.WISE_ORG,
    orgUnit: env.WISE_ORG_UNIT,
    joinPolicy: 'JOINPWD',
    joinPwd: env.SESSION_JOIN_PWD,
    lobbyEnabled: true,
  })
...
app.post('/join', async (request: FastifyRequest, reply: FastifyReply) => {
  const { session, firstName, lastName, moderator } = request.body as JoinSessionBody
  ...
  const joinResponse = await WiseAPI.session.join(session, {
    firstName,
    lastName,
    moderator,
    password: env.SESSION_JOIN_PWD,
  })

  console.log(joinResponse)

  reply.status(200).send({
    sessionId: joinResponse.short,
